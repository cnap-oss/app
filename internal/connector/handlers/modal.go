...
func (h *DiscordHandler) handleModal(i *discordgo.InteractionCreate) {
	...
	case customID == prefixModalCreate:
		...
		if err := h.controller.CreateAgent(ctx, name, desc, "opencode", model, prompt); err != nil {
			h.logger.Error("Failed to create agent via controller", zap.Error(err))
			h.respondEphemeral(i, fmt.Sprintf("오류: 에이전트 '**%s**'을(를) 생성하는 데 실패했어요. 에러: %v", name, err))
			return
		}
		h.respondEphemeral(i, fmt.Sprintf("에이전트 '**%s**'이(가) 성공적으로 생성되었어요!", name))
	case strings.HasPrefix(customID, prefixModalEdit):
		originalName := strings.TrimPrefix(customID, prefixModalEdit)
		// Discord 봇에서는 기본 provider로 "opencode" 사용
		if err := h.controller.UpdateAgent(ctx, originalName, desc, "opencode", model, prompt); err != nil {
			h.logger.Error("Failed to update agent via controller", zap.Error(err), zap.String("original_agent_id", originalName))
			h.respondEphemeral(i, fmt.Sprintf("오류: 에이전트 '**%s**'을(를) 수정하는 데 실패했어요. 에러: %v", originalName, err))
			return
		}
		h.respondEphemeral(i, fmt.Sprintf("에이전트 '**%s**'의 정보가 성공적으로 수정되었어요!", name))
	}
}

// showCreateOrEditModal은 에이전트 생성/수정 모달을 표시합니다.
func (h *DiscordHandler) showCreateOrEditModal(i *discordgo.InteractionCreate, originalName string, agent *controller.AgentInfo) {
	...
	err := h.session.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{Type: discordgo.InteractionResponseModal, Data: modal})
	if err != nil {
		h.logger.Error("Failed to show create/edit modal", zap.Error(err))
	}
}
